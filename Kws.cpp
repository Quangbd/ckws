...
bool Kws::wakeup(const short *short_input_buffer, int length) {
    ...
    for (int i = 0; i < length; i++) {
        float float_input = ((float) short_input_buffer[i] / 32767.0f);
    ...
    TfLiteTensorCopyFromBuffer(input_tensor,
                               &float_input_buffer[0], input_tensor->bytes);
...
//    LOG_INFO("infer %l", current_timestamp - start);
    ...
    if (is_new_command) {
        ...
        return false;
               ...
               && ((float) (current_timestamp - wakeup_queue_timestamps[0]) > 1)) {
        wakeup_queue_timestamps.pop_front();
        wakeup_queue_scores.pop_front();
    }

    if ((output[1] > output[0]) && ((float) (current_timestamp - previous_wakeup_time) > 1.5)) {
        ...
        if (!is_new_command && (wakeup_queue_scores.size() > 5) && (avg_score > 0.75)) {
            LOG_DEBUG("Wakeup core N_%f - P_%f - %lu", avg_score);
